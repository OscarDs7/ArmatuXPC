...
function Modelo({ trigger }) {
  ...
  const group = useRef();
  const { scene, animations } = useGLTF("/models/CPU.glb");
...
const pasosInstalar = [
    ...
    "Localizar el socket de la CPU en la placa madre.",
    "Levantar la palanca del socket para abrirlo.",
    "Colocar cuidadosamente el procesador en el socket alineando las marcas.",
    "Bajar la palanca del socket para asegurar la CPU.",
    "Aplicar pasta térmica sobre la CPU si es necesario.",
    "Colocar el disipador/ventilador sobre la CPU y asegurarlo según instrucciones.",
    "Conectar el cable del ventilador de la CPU al conector correspondiente en la placa madre.",
    "Cerrar el gabinete.",
    "Conectar el cable de corriente y encender el equipo para verificar funcionamiento."
  ];

  // 🔴 PASOS PARA DESINSTALAR CPU
  const pasosDesinstalar = [
    "Apagar completamente la computadora y desconectar el cable de corriente.",
    "Abrir el gabinete retirando el panel lateral.",
    "Desconectar el cable del ventilador de la CPU de la placa madre.",
    "Retirar el disipador/ventilador de la CPU.",
    "Levantar la palanca del socket de la CPU para desbloquearla.",
    "Extraer cuidadosamente la CPU del socket.",
    "Si es necesario, limpiar la pasta térmica de la CPU y del disipador.",
    "Cerrar el gabinete."
  ];
